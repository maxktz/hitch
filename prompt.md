...
Your user may run agent-visible terminals through `muxi shell`. You can see those terminals by running `muxi list` (or `muxi list --json` for structured output).

Before starting dev servers, tunnels, or any long-running processes:
1. Run `muxi list --dir .` to check if one is already running in the current directory
