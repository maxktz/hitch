...
/* Tracks OSC title sequences that may span multiple PTY reads. */
static int title_filter_state;
...
static int
is_title_osc_command(unsigned char c)
{
	return c == '0' || c == '1' || c == '2';
}

static ssize_t
filter_title_sequences(unsigned char *dst, const unsigned char *src, ssize_t len)
{
	ssize_t i, out = 0;
	...
	for (i = 0; i < len; i++)
	{
		...
		if (title_filter_state == 0)
		{
			if (c == '\033')
			{
				title_filter_state = 1;
				continue;
			}
			dst[out++] = c;
		}
		else if (title_filter_state == 1)
		{
			if (c == ']')
				title_filter_state = 2;
			else
			{
				dst[out++] = '\033';
				dst[out++] = c;
				title_filter_state = 0;
			}
		}
		else if (title_filter_state == 2)
		{
			if (is_title_osc_command(c))
				title_filter_state = 3;
			else
			{
				dst[out++] = '\033';
				dst[out++] = ']';
		...
		else if (title_filter_state == 3)
		{
			if (c == ';')
				title_filter_state = 4;
			else
			{
				dst[out++] = '\033';
				dst[out++] = ']';
		...
		else if (title_filter_state == 4)
		{
			if (c == '\007')
				title_filter_state = 0;
			else if (c == '\033')
				title_filter_state = 5;
		}
		else if (title_filter_state == 5)
		{
			if (c == '\\')
				title_filter_state = 0;
			else
...
init_pty(char **argv, int statusfd)
{
	...
	** window size here, because the attacher will send it in a packet. */
	...
	write_pid_file(getenv("MUXI_PID_FILE"), the_pty.pid);
#ifdef BROKEN_MASTER
	{
		char *buf;
...
pty_activity(int s)
{
	...
	unsigned char filtered[BUFSIZE];
	ssize_t len;
	...
	filtered_len = filter_title_sequences(filtered, buf, len);

	if (muxi_log_fd >= 0)
		write(muxi_log_fd, filtered, filtered_len);
...
top:
	...
	*/
	...
	for (p = clients, nclients = 0; p; p = p->next)
	{
		...
		{
			ssize_t n = write(p->fd, filtered + written, filtered_len - written);
		...
		if (nclients != -1 && written == filtered_len)
			nclients++;
	}
...
master_main(char **argv, int waitattach, int dontfork)
{
	...
	** that occur while trying to execute the program. */
	...
	if (dontfork)
	{
		write_pid_file(getenv("MUXI_MASTER_PID_FILE"), getpid());
	...
	/* Parent - just return. */
	write_pid_file(getenv("MUXI_MASTER_PID_FILE"), pid);
