...
use cli::{CapturePaneArgs, Cli, Commands, ContextArgs, StatusArgs};
use filters::{AltScreenLogFilter, AltScreenTracker, TitleFilter};
use models::{SessionRecord, SessionState};
use render::render_terminal_text;
use send_keys::cmd_send_keys;
use setup::{
    cmd_print_skill, cmd_setup, cmd_welcome_setup, ensure_shell_integration,
    maybe_refresh_update_cache, outdated_skill_warning, update_warning,
};
use style::Style;

const BUF_SIZE: usize = 4096;
pub(crate) const MSG_PUSH: u8 = 0;
const MSG_ATTACH: u8 = 1;
const MSG_DETACH: u8 = 2;
const MSG_WINCH: u8 = 3;
const MSG_DETACH_SESSION: u8 = 4;
pub(crate) const SKILL_NAME: &str = "hitch";
pub(crate) const SKILL_VERSION: &str = "3";
pub(crate) const SKILL_MD: &str = include_str!("../SKILL.md");
pub(crate) const HITCH_VERSION: &str = env!("CARGO_PKG_VERSION");
pub(crate) const INSTALL_SOURCE: &str = match option_env!("HITCH_INSTALL_SOURCE") {
    Some(source) => source,
    None => "dev",
};
pub(crate) const NPM_PACKAGE_NAME: &str = "hitch-cli";
pub(crate) const UPDATE_CACHE_TTL_SECS: u64 = 6 * 60 * 60;
pub(crate) const NPM_REGISTRY_URL: &str = "https://registry.npmjs.org/hitch-cli";
...
pub(crate) fn update_cache_path() -> PathBuf {
...
pub(crate) fn file_len(path: &str) -> u64 {
    fs::metadata(path)
...
pub(crate) fn send_packet(stream: &mut UnixStream, typ: u8, payload: &[u8]) -> io::Result<()> {
...
pub(crate) fn print_context_session_to_stderr(
    session: &SessionRecord,
    head_limit: usize,
    tail_limit: usize,
    no_output: bool,
...
pub(crate) fn find_session(id: Option<&str>) -> io::Result<SessionRecord> {
...
pub(crate) fn read_session_state(id: &str) -> SessionState {
...
pub(crate) fn rendered_lines_from_offset(path: &str, offset: u64, limit: usize) -> Vec<String> {
